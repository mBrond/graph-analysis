...
class Graph:
    ...
    def bellman_ford(self, start):
        ...
        relaxation_order = ['S', 'A', 'B', 'C', 'D', 'E']

        # Relax edges in the specified order
        for i in range(num_vertices - 1):
            for u in relaxation_order:
                if u in self.graph:  # Check if the node exists in the graph
...
if __name__ == "__main__":
    ...
    # Running the Bellman-Ford algorithm from node 'S'
    ...
    plt.show()  # Uncomment this line if you want to display the final visualization